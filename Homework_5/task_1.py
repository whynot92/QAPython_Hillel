...
for model, info in car_data.items():
    year = info[1]
    engine = info[2]
    price = info[4]
    
    if year >= min_year and engine >= min_engine and price <= max_price:
        filtered_cars.append((model, info))

for i in range(len(filtered_cars)):
    for j in range(i + 1, len(filtered_cars)):
        if filtered_cars[i][1][4] > filtered_cars[j][1][4]:
            filtered_cars[i], filtered_cars[j] = filtered_cars[j], filtered_cars[i]

count = 0
for model, info in filtered_cars:
    if count < 5:
        print(f'{model}: Color: {info[0]}, Year: {info[1]}, Engine: {info[2]}, Type: {info[3]}, Price: {info[4]}')
        count += 1
    else:
        break